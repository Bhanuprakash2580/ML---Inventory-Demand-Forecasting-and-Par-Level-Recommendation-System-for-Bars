...
try:
    ...
    st.error("Generated outputs are missing. Run all cells in the notebook first.")
    st.stop()
...
with right:
    ...
    display_metrics = metrics.rename(
        index={"Seasonal naive (7 days)": "Seasonal naive", "Random Forest": "Random Forest"}
    ).copy()
...
for column in [
    "predicted_daily_demand_ml", "safety_stock_ml", "par_level_ml",
    "lost_volume_ml", "average_inventory_ml",
]:
...
st.caption(
    f"Inventory position is simulated with a 2-day lead time and a 95% service-level safety buffer. "
    f"Total average inventory across selected series: {average_inventory:,.0f} ml."
)
